...
- `PlayerController` drives the shared player: on `load()` it sets a `LockCachingAudioSource` (with a `MediaItem` tag — required for lock-screen/background metadata), seeks to the story's resume position, then wires listeners for throttled position persistence and completion. **Saving is explicit**: a freshly generated story is an in-memory draft; it reaches the Bookshelf only when the user taps **Save to Bookshelf** on the Finished screen (`PlayerController.save()`). The mini player's **×** (`discard()`) drops an unsaved draft with nothing to delete. `_saved` initializes in `load()` from repo membership, so replaying an already-saved story keeps persisting its resume position.
- The **mini player** (`lib/widgets/mini_player.dart`) is the now-playing bar shown in the `Scaffold.bottomNavigationBar` of Home/Bookshelf/Settings. It reads `currentStoryProvider` **first** and renders `SizedBox.shrink()` when idle, so it never constructs the audio player when no story is loaded (keeps the headless smoke test green). Tapping the body reopens the Player; the Player's `initState` guards `load()` on story id so reopening attaches to live playback instead of restarting.
...
- Android: `MainActivity` **must extend `AudioServiceActivity`** (not `FlutterActivity`) — `just_audio_background`/`audio_service` rely on a cached FlutterEngine, and `JustAudioBackground.init()` throws "The Activity class ... is wrong" without it, which (being awaited before `runApp`) blanks the whole app. The manifest declares the `just_audio_background` service/receiver + `WAKE_LOCK`/`FOREGROUND_SERVICE`/`FOREGROUND_SERVICE_MEDIA_PLAYBACK` and `RECORD_AUDIO`/`INTERNET`; iOS `Info.plist` has the `audio` background mode + mic/speech usage strings. Screen-off playback + lock-screen controls still need a real-device check.
- Any plugin init awaited before `runApp` in `main()` (Hive, `JustAudioBackground.init`) is a footgun: if it throws, `runApp` never runs and you get a silent blank screen (the error only shows in `flutter run`/logcat, and debug can mask what release surfaces). When adding startup init, keep this in mind.
